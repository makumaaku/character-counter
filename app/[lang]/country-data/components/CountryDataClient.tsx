...
// クライアントコンポーネントに渡される翻訳データの型
interface CountryDataClientProps {
  messages: {
    title: string;
    states: {
      loading: string;
...
export default function CountryDataClient({ messages }: CountryDataClientProps) {
  ...
  if (loading) {
    return <div className="text-center my-8 font-bold text-gray-600">{messages.states.loading}</div>;
  }

  if (error) {
    return <div className="text-center my-8 font-bold text-red-600">{messages.states.error}</div>;
  }

  return (
    <main className="p-4 max-w-7xl mx-auto bg-white dark:bg-white shadow-lg rounded-lg">
      <h1 className="text-2xl font-bold mb-6 text-gray-900">{messages.title}</h1>
      
      <nav className="flex flex-wrap gap-4 items-center mb-6">
        <div>
          <label htmlFor="searchInput" className="mr-2 text-gray-900">{messages.search.label}</label>
          <input
            type="text"
            id="searchInput"
        ...
        <div>
          <label htmlFor="regionFilter" className="mr-2 text-gray-900">{messages.filter.label}</label>
          <select
            id="regionFilter"
            className="px-3 py-1 border border-gray-300 rounded text-gray-900 bg-white"
          ...
          >
            <option value="all">{messages.filter.allRegions}</option>
      ...
      {filteredData.length === 0 ? (
        <p className="text-red-600 font-bold">{messages.states.noData}</p>
      ) : (
