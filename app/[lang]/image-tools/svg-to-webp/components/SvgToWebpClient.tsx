...
export default function SvgToWebpClient({ translations }: Props) {
  ...
  useEffect(() => {
    return () => {
      ...
      convertedFiles.forEach(file => {
        if (!file.webpUrl.startsWith('data:')) {
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        try {
          ...
          // SVGをWebPに変換 (固定の品質と幅を使用)
          const webpBlob = await svgToWebp(svgText, file.name)
  ...
  // SVGをWebPに変換する関数
  const svgToWebp = (svgText: string, fileName: string): Promise<Blob> => {
    // 固定値を使用
    const width = 1200 // 1200pxの幅
    const quality = 0.9 // WebP品質 (0.0 ~ 1.0)
      ...
      try {
        ...
        img.onload = () => {
          try {
            ...
            // WebP形式でも透明背景を維持するため、背景色は塗りつぶさない
            // (SVGの透過を維持するため)
  ...
  const downloadCurrentFile = () => {
    ...
    saveAs(file.webpBlob, `${file.originalName}.webp`)
  }
  ...
  const downloadAllFiles = () => {
    ...
    if (convertedFiles.length === 1) {
      ...
      convertedFiles.forEach(file => {
        zip.file(`${file.originalName}.webp`, file.webpBlob)
      })
      
      zip.generateAsync({ type: 'blob' }).then(content => {
        saveAs(content, 'svg-to-webp-converted.zip')
      })
    }
  }
        ...
        <div className="mt-6 bg-gray-700 p-4 rounded-lg">
          ...
          <div className="bg-gray-600 p-2 rounded-lg mb-4">
            <div className="aspect-auto w-full flex items-center justify-center bg-gray-800 rounded overflow-hidden">
              <Image
                src={convertedFiles[currentPreviewIndex]?.webpUrl}
                alt="Converted WebP"
